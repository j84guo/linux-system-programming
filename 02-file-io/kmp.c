...
void kmp_search(int *prefix_table, char *pattern, int m,
...
int read_str_buf(char *path, struct StringBuffer *sb)
{
	...
	int capacity = READ_SIZE, n = 0, r = 0;
	sb->text = malloc(capacity + 1);
	...
	while ((r = read(fd, sb->text + n, READ_SIZE)) != 0) {
		...
		if (capacity < n + READ_SIZE) {
			capacity *= 2;
			sb->text = realloc(sb->text, capacity + 1);
		}
	}
